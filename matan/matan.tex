...
\begin{definition}
    ...
    Для таких $x(t)$ рассмотрим функцию $\phi(t) = f(x(t))$. Если $\exists \dslim_{t \to 0 + 0} \phi(t) = b$,
...
\begin{example}
    ...
    \]
    \[
        \implies \exists \dslim_{t \to +0} \phi(t) = 
...
\begin{definition}
    Говорят, что $f(x)$ имеет порядок функции (асимптотику) $\phi(x)$ на множестве $A$,
    если $\exists C > 0:\: \forall x \in A$ ($C$ -- const) выполняется
    $|f(x)| \le C*|\phi(x)|$. (Или если при $x \approach{} a$ существует окрестность точки $a$, 
    в которой выполняется $|f(x)| \le C*|\phi(x)|$), и обозначают $f(x) = O (\phi(x))$ 
...
\begin{remark}
    \begin{enumerate}
        ...
        \item Если на множестве $A$ $f(x) = O(\phi_1(x)), \; \phi_1(x) = O(f_2(x)) \implies f(x) = O (\phi_2(x))$ на $A$
    \end{enumerate}
\end{remark}

\begin{definition}
    Говорят, что $f(x)$ имеет порядок малости функции $\phi(x)$ при $x \approach{} a$, 
    если $\exists \eps(x):\: f(x) = \eps(x) * \phi(x)$ и $\dslim_{x \to a} \eps(x) = 0$ 
    (т.е. $\dslim_{x \to a} \frac{f(x)}{\phi(x)} = 0$), и обозначают 
    $f(x) = o(\phi(x))$ при $x \approach{} a$
\end{definition}
...
\begin{enumerate}
    \item 
        Если 
        $\begin{cases}
            f(x) = O(\phi(x)) &\textnormal{ при } x \to a \\
            g(x) = o(\phi(x)) &\textnormal{ при } x \to a
        \end{cases}$ 
        $\implies f(x) + g(x)= O(\phi(x))$ при $x \to a$
    
    \item 
        Если $f(x) = o(\phi_1(x))$ и $\phi_1(x) = o(\phi_2(x))$ при 
        $x \approach{} a \implies f(x) = o(\phi_2(x))$ при $x \to a$
    
    \item Если $f(x) = O(\phi(x))$ при $x \to a$, то $f(x) * g(x) = O(\phi(x) * g(x))$ при $x \to a$
    \item Если $f(x) = o(\phi(x))$ при $x \to a$, то $f(x) * g(x) = o(\phi(x) * g(x))$ при $x \to a$
...
\begin{theorem}[Свойства предела функции]
    \begin{enumerate}
        ...
        \item 
            Пусть $t = \phi(x)$ и $y = f(t)$ заданы так, что $D_y = A$, $E_g = B$, $D_f = B$. 
            Пусть $\exists \dslim_{x \to a} \phi(x) = b$ и $\exists \dslim_{t \to b} f(t) = C$, $\phi(x) \ne b$ при $x \ne a$.
            Тогда $\exists \dslim_{x \to a} f(\phi(x)) = C$
    \end{enumerate}
\end{theorem}
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \setcounter{enumi}{5} \item 
            ...
            $t_n = \phi(X_n) \approach{} b$

            $t_n \in B,\, t_n \neq b$ $\implies$ (по опр Гейне) $f(t_n) \approach{} C$, 

            т.е. $f(\phi(x_n)) \approach{} C$ 
            $\implies$ (по опр Гейне) $\exists \dslim_{x \to a} f(\phi(x)) = C$
    \end{enumerate}
\end{proof}
